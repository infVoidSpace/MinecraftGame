    ...
    public void OnEndDrag(PointerEventData data)
    {
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            else
            {
                ...
                dropItem.transform.parent = GameObject.FindGameObjectWithTag("Drops").transform;
                if (dropItem.GetComponent<PickUpItem>()==null)
                {
                    dropItem.AddComponent<PickUpItem>();
                }
                dropItem.GetComponent<Rigidbody>();
                dropItem.GetComponent<PickUpItem>().item = this.gameObject.GetComponent<ItemOnObject>().item;
                
                dropItem.transform.position = GameObject.FindGameObjectWithTag("Player").transform.position + GameObject.FindGameObjectWithTag("Player").transform.forward + new Vector3(0, 1, 0);
                ...
                dropItem.GetComponent<PickUpItem>().item.itemValue = this.gameObject.GetComponent<ItemOnObject>().item.itemValue;
            }
        }
        inventory.OnUpdateItemList();
    }
